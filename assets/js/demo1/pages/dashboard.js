...
var KTDashboard = function() {
    ...
    var _initSparklineChart = function(src,datax, datay, color, border) {
        if (src.length == 0) {
            return;
        }
    ...
    var salesByApps = function() {
        ...
        var rangoFecha1 = moment().format('YYYY-MM-DD'),
        rangoFecha2 = moment().format('YYYY-MM-DD');
        graficos(rangoFecha1,rangoFecha2);
        
                _initSparklineChart($('#kt_chart_sales_by_apps_1_22'),[2, 16, 0, 12, 22, 5, -10, 5, 15, 2,1,2], [2, 16, 0, 12, 22, 5, -10, 5, 15, 2], '#FA7255', 2);
                _initSparklineChart($('#kt_chart_sales_by_apps_1_23'),[2, 16, 0, 12, 22, 5, -10, 5, 15, 2,1,2], [2, 16, 0, 12, 22, 5, -10, 5, 15, 2], '#FF0000', 2);
                _initSparklineChart($('#kt_chart_sales_by_apps_1_24'),[2, 16, 0, 12, 22, 5, -10, 5, 15, 2,1,2], [2, 16, 0, 12, 22, 5, -10, 5, 15, 2], '#0BDCE2', 2);
        $('#kt_dashboard_daterangepicker').on('apply.daterangepicker', function(ev, picker) {
            rangoFecha1 = picker.startDate.format('YYYY-MM-DD');
            rangoFecha2 = picker.endDate.format('YYYY-MM-DD');
            graficos(rangoFecha1,rangoFecha2);
            _initSparklineChart($('#kt_chart_sales_by_apps_1_22'),[2, 16, 0, 12, 22, 5, 10, 5, 15, 2,1,2], [2, 16, 0, 12, 22, 5, -10, 5, 15, 2], '#FA7255', 2);
            _initSparklineChart($('#kt_chart_sales_by_apps_1_23'),[2, 16, 0, 12, 22, 5, 10, 5, 15, 2,1,2], [2, 16, 0, 12, 22, 5, -10, 5, 15, 2], '#FF0000', 2);
            _initSparklineChart($('#kt_chart_sales_by_apps_1_24'),[2, 16, 0, 12, 22, 5, 10, 5, 15, 2,1,2], [2, 16, 0, 12, 22, 5, -10, 5, 15, 2], '#0BDCE2', 2);
    ...
    var graficos = function(fecha1,fecha2){
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.response.forEach(element => {
                    dataY.push(element.vPromedio)
                    dataX.push(element.fecha)
                });
                _initSparklineChart($('#kt_chart_sales_by_apps_1_21'),dataX, dataY, "#5573DE", 2);
    ...
    function setNumber(numero) { 
        var numeros = [];
        total = numero.toString();
        for(var i=0;total.toString().length>3;i++){
            numeros[i] = total.slice(-3).toUpperCase();
            total = total.slice(0, -3);
        }
        var numSet= '';
        var numI = numeros.length-1;
    
        for(var j=numI;j>=0;j--){
            numSet= numSet+'.'+numeros[j];
        }
        numSet = total+numSet;
        return numSet;
    }
    ...
    // Order Statistics.
    // Based on Chartjs plugin - http://www.chartjs.org/
    var orderStatistics = function(datosX,datosY) {
        ...
        var barChartData = {
            ...
            datasets : [
				/* {
                    fill: true,
                    //borderWidth: 0,
                    backgroundColor: color("#23C4CE").alpha(0.2).rgbString(),
                    ...
                    pointBackgroundColor: Chart.helpers.color('#000000').alpha(0).rgbString(),
                    pointBorderColor: Chart.helpers.color('#000000').alpha(0).rgbString(),
                    pointHoverBackgroundColor: '#23C4CE',
                    pointHoverBorderColor: Chart.helpers.color('#000000').alpha(0.1).rgbString(),
				...
				}, */
				{
                    fill: true,
                    //borderWidth: 0,
					backgroundColor : color('#0B3CE2').alpha(0.2).rgbString(),
                    borderColor : color(KTApp.getStateColor('brand')).alpha(0).rgbString(),
                    ...
                    pointBackgroundColor: Chart.helpers.color('#000000').alpha(0).rgbString(),
                    pointBorderColor: Chart.helpers.color('#000000').alpha(0).rgbString(),
                    pointHoverBackgroundColor: "#0B3CE2",
                    pointHoverBorderColor: Chart.helpers.color('#000000').alpha(0.1).rgbString(),
    ...
    var daterangepickerInit = function() {
        ...
        function cb(start, end, label) {
            ...
            if ((end - start) < 100 || label == 'Hoy') {
                ...
                start['_locale']['_monthsShort'][0] = 'Ene';
                start['_locale']['_monthsShort'][1] = 'Feb';
                start['_locale']['_monthsShort'][2] = 'Mar';
                start['_locale']['_monthsShort'][3] = 'Abr';
                start['_locale']['_monthsShort'][4] = 'May';
                start['_locale']['_monthsShort'][5] = 'Jun';
                start['_locale']['_monthsShort'][6] = 'Jul';
                start['_locale']['_monthsShort'][7] = 'Ago';
                start['_locale']['_monthsShort'][8] = 'Sep';
                start['_locale']['_monthsShort'][9] = 'Oct';
                start['_locale']['_monthsShort'][10] = 'Nov';
                start['_locale']['_monthsShort'][11] = 'Dic';
